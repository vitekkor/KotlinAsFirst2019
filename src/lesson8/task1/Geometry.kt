...
fun circleByDiameter(diameter: Segment): Circle {
    val begin = diameter.begin
    val end = diameter.end
    val centerX = (begin.x + end.x) / 2.0
    val centerY = (begin.y + end.y) / 2.0
    return Circle(Point(centerX, centerY), begin.distance(end) / 2.0)
}
...
class Line private constructor(val b: Double, val angle: Double) {
    ...
    fun crossPoint(other: Line): Point {
        val sinDifference = sin(angle - other.angle)
        val x = (other.b * cos(angle) - b * cos(other.angle)) / sinDifference
        val y = (b * sin(other.angle) - other.b * sin(angle)) / sinDifference * -1.0
        return Point(x, y)
    }
...
fun lineBySegment(s: Segment): Line =
    Line(s.begin, (atan((s.end.y - s.begin.y) / (s.end.x - s.begin.x)) + 2 * PI) % PI)
//+ 2 * PI и % PI из-за условия angle >= 0 && angle < PI
...
fun bisectorByPoints(a: Point, b: Point): Line =
    Line(Point((b.x + a.x) / 2, (b.y + a.y) / 2), (lineByPoints(a, b).angle + PI / 2) % PI)
...
fun minContainingCircle(vararg points: Point): Circle {
    ...
    return if (points.size == 1) Circle(points[0], 0.0) else {
        ...
        for (p in points) {
            if (!circle.contains(p) && p != p1 && p != p2) circle = circleByThreePoints(p1, p2, p)
        }
        circle
    }
}
...
fun main() {
    val points = arrayOf(
        Point(0.9719249837306013, 0.489637775345519),
        Point(0.4189205152353913, 0.27297895333269817),
        Point(-632.0, -632.0),
        Point(0.0, 2.220446049250313e-16),
        Point(0.8602728764670975, -632.0),
        Point(2.220446049250313e-16, -5e-324),
        Point(0.0, 0.3880591108951499),
        Point(-632.0, -632.0),
        Point(0.7342804488506111, 0.14417984519255322),
        Point(0.13140527195515916, 0.09755409672075155)
    )
    println(minContainingCircle(*points))
}
