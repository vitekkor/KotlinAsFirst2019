...
class PhoneBook {
     ...
     */
    fun addHuman(name: String): Boolean = if (book[name] == null) {
     ...
     * (во втором случае телефонная книга не должна меняться).
     */
    fun removeHuman(name: String): Boolean = if (book[name] == null) false else {
        book.remove(name)
        true
    }
    ...
    fun addPhone(name: String, phone: String): Boolean =
        if (book[name] == null || book.any { it.value.indexOf(phone) != -1 }) false else {
            book[name]!!.add(phone)
            true
        }
    ...
    fun removePhone(name: String, phone: String): Boolean =
        if (book[name] == null || book[name]!!.indexOf(phone) == -1) false else {
     ...
     */
    fun phones(name: String): Set<String> = book[name]?.toSet() ?: setOf()
    ...
    fun humanByPhone(phone: String): String? {
        for ((human, number) in book) {
            if (number.indexOf(phone) != -1) return human
        }
        return null
    }
    ...
    override fun hashCode(): Int {
        var result = 1
        for ((human, number) in book) {
            result += human.hashCode() + number.fold(0) { sum, it -> sum + it.hashCode() }
        }
        return result
    }
}